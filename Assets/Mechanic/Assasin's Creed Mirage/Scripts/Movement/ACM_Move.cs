...
public class ACM_Move : MonoBehaviour
{
    ...
    public void ExitMovement()
    {
        ...
        _input.sprint = false;
        float initialSpeed = _animator.GetFloat(_animIDSpeed);
        if (_hasAnimator)
        {
            StartCoroutine(SmoothStop(initialSpeed));
        }
        
    }

    private IEnumerator SmoothStop(float initialSpeed)
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            float newSpeed = Mathf.Lerp(initialSpeed, 0f, t);
                _animator.SetFloat(_animIDSpeed, newSpeed);
            yield return null;
        }
        _animator.SetFloat(_animIDSpeed, 0f);
    }
    public void HandleCameraLateUpdate()
    {
        CameraRotation();
    }
